...
/// For beacon offers, there may be navigations like open Safari browser, open a url in a webview etc. If the application is in background, SDK displays the offer data in local notifications. The host app must call applicationDidReceiveLocalNotification for every notification tap (including when app is in foreground); otherwise tapping the notification in foreground will not open the LP/external link. The API has to be called in the “userNotificationCenter: didReceiveNotificationResponse: withCompletionHandler:” callback of AppDelegate. The SSBP SDK notification identifier will contain the string “SSBPSDKNotification”.
/// @param response notification's response
/// @note Identifier contains "SSBPSDKNotification" or "SSBPSDKAreaNotification". For foreground banner, implement willPresentNotification and return UNNotificationPresentationOptionAlert.
